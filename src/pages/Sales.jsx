...
export default function Sales() {
  ...
  function addToCart(p) {
    ...
    if (exists) {
      setCart(cart.map(c => c.id === p._id ? { ...c, qty: Math.min(c.qty + 1, p.stock) } : c))
    } else {
      setCart([...cart, { id: p._id, name: p.name, price: p.price, qty: 1 }])
    }
  }

  function setQty(id, qty) {
    setCart(cart.map(c => c.id === id ? { ...c, qty: Math.max(1, Number(qty) || 1) } : c))
  }
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-2">
            {products.map(p => (
              <button
                key={p._id}
                onClick={() => addToCart(p)}
                className="border rounded p-3 text-left hover:bg-orange-50"
              >
